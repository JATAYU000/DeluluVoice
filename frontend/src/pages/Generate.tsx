...
export default function Generate() {
  ...
  const [useInstruments, setUseInstruments] = useState(
    generationState.useInstruments ?? true,
  );
  const [aiEnhancedLyrics, setAiEnhancedLyrics] = useState(
    generationState.aiEnhancedLyrics ?? false,
  );
  ...
  const handleStart = async () => {
    ...
    try {
      if (aiEnhancedLyrics) {
        const response = await fetch("http://localhost:8000/rewrite-lyrics", {
          method: "POST",
          credentials: "include",
          headers: {
    ...
    <div className="min-h-screen bg-[#050505] text-white p-4 md:p-12 font-sans flex flex-col items-center">
      ...
      <header className="w-full max-w-6xl flex justify-between items-center mb-12">
        ...
        >
          ...
          <span className="font-mono text-[10px] md:text-sm tracking-widest uppercase">
            Back to Studio
          </span>
        </Link>
      ...
      <div className="w-full max-w-6xl flex-1 flex flex-col lg:flex-row gap-12 lg:gap-20">
        ...
        <div className="flex-1 flex flex-col relative z-10 w-full">
          <div className="flex justify-between items-center mb-6">
            <h1 className="font-display font-black text-3xl md:text-4xl tracking-tighter uppercase">
              New Track
            </h1>
            <button
            ...
            {showInfo && (
              ...
              >
                <div className="mb-6 bg-orange-500/10 border border-orange-500/30 rounded-xl p-6">
                  <h3 className="font-display font-bold text-orange-400 mb-2 uppercase tracking-wide">
                    Writing Guide
                  </h3>
                  <p className="text-sm text-white/70 mb-4 font-mono leading-relaxed">
                    Write your lyrics exactly how you want them performed. Use
                    bracketed tags [Verse], [Chorus], or [Drop] to guide the
                    flow.
                  </p>
                  <div className="bg-black/50 p-4 rounded text-xs font-mono text-white/50 whitespace-pre-line border border-white/5 leading-relaxed">
                    ...
                    {"\n"}
                    Entrepreneur, fresh out the sewer, watch me maneuver, what's
                    it to ya?
                  </div>
                </div>
              </motion.div>
            ...
            <div className="border-t border-white/10 p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center bg-black/40 rounded-b-xl gap-4">
              <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 items-start sm:items-center w-full sm:w-auto">
                <label className="flex items-center gap-3 cursor-pointer group">
                  ...
                  />
                  <div
                    className={`w-12 h-6 rounded-full transition-colors relative flex items-center ${useInstruments ? "bg-orange-500" : "bg-white/10"}`}
                  >
                    <motion.div
                      className="w-4 h-4 rounded-full bg-white absolute shadow-md"
                      animate={{
                        left: useInstruments ? "calc(100% - 20px)" : "4px",
                      }}
                      transition={{
                        type: "spring",
                        stiffness: 500,
                  ...
                  <span
                    className={`font-mono text-xs tracking-widest uppercase transition-colors flex items-center gap-2 ${useInstruments ? "text-white" : "text-white/40"}`}
                  >
                    Instruments
                    <Mic2
                      className={`w-4 h-4 ${useInstruments ? "text-orange-500" : "text-white/20"}`}
                    />
                  </span>
                </label>
                ...
                <label className="flex items-center gap-3 cursor-pointer group">
                  ...
                  />
                  <div
                    className={`w-12 h-6 rounded-full transition-colors relative flex items-center ${aiEnhancedLyrics ? "bg-orange-500" : "bg-white/10"}`}
                  >
                    <motion.div
                      className="w-4 h-4 rounded-full bg-white absolute shadow-md"
                      animate={{
                        left: aiEnhancedLyrics ? "calc(100% - 20px)" : "4px",
                      }}
                      transition={{
                        type: "spring",
                        stiffness: 500,
                  ...
                  <span
                    className={`font-mono text-xs tracking-widest uppercase transition-colors flex items-center gap-2 ${aiEnhancedLyrics ? "text-white" : "text-white/40"}`}
                  >
                    AI Enhanced Lyrics
                    <Wand2
                      className={`w-4 h-4 ${aiEnhancedLyrics ? "text-orange-500" : "text-white/20"}`}
                    />
                  </span>
                </label>
              </div>
        ...
        <div className="w-full lg:flex-[0.8] flex flex-col items-center justify-center relative perspective-[1000px]">
          ...
          {generationState.status === "idle" && (
            ...
            >
              ...
              <h2 className="font-display font-black text-xl md:text-2xl uppercase tracking-tighter mb-2">
                Ready to Record
              </h2>
          ...
          {generationState.status === "generating" && (
            ...
            >
              ...
              <div className="relative z-10 flex flex-col items-center w-full">
                ...
                <h2 className="font-display font-black text-2xl md:text-3xl uppercase tracking-tighter text-orange-500 animate-pulse mb-1 text-center">
                  Synthesizing
                </h2>
                <p className="font-mono text-white/50 text-[10px] md:text-xs tracking-widest uppercase mb-6 text-center">
          ...
          {generationState.status === "complete" && (
            ...
            >
              ...
              <h2 className="font-display font-black text-2xl uppercase tracking-tighter mb-4 text-white">
                Track Mastered
              </h2>
              ...
              <div className="w-full bg-black/50 p-6 rounded-xl border border-white/10 mb-6 flex flex-col gap-4">
                ...
                <div>
                  ...
                  <label className="flex items-center gap-3 cursor-pointer group w-max">
                    ...
                    />
                    <div
                      className={`w-12 h-6 rounded-full transition-colors relative flex items-center ${isPublic ? "bg-orange-500" : "bg-white/10"}`}
                    >
                      <motion.div
                        className="w-4 h-4 rounded-full bg-white absolute shadow-md"
                        animate={{
                    ...
                    <span
                      className={`font-mono text-xs tracking-widest uppercase transition-colors ${isPublic ? "text-white" : "text-white/40"}`}
                    >
          ...
          {generationState.status === "error" && (
            ...
            >
              ...
              <h2 className="font-display font-black text-2xl uppercase tracking-tighter mb-2 text-red-400">
                Generation Failed
              </h2>
